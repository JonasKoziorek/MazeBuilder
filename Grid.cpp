...
AdList Grid::CreateGraph(){
    ...
    int n = this->width;
    int m = this->height;
    vector<vector<int>> adlist(m*n, vector<int>());
    for(int i = 1; i < m-1; i++){
        // interior cells
        for(int j = 1; j < n-1; j++){
    ...
    // bottom leff corner
    adlist[(m-1)*m] = {this->FormatIndex(m-2, 0),this->FormatIndex(m-1, 1)};
    // bottom right corner
    adlist[(m-1)*m + n-1] = {this->FormatIndex(m-1, n-2),this->FormatIndex(m-2, n-1)};
